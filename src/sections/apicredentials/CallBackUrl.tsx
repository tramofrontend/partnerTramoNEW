...
export default function CallBackUrl() {
  ...
  const setMoneyurl = () => {
    if (!edit1) {
      ...
      Api(`apiBox/dashboard/updateMoneyTransferCallbackUrl`, 'POST', body, token).then(
        (Response: any) => {
          console.log('======userProfileInfo==response=====>', Response);
          if (Response.status == 200) {
            if (Response.data.code == 200) {
              ...
              getBBPSUurl();
              console.log(
                '======updateMoneyTransferCallbackUrl==code 200=====>',
                Response.data.data
              );
            } else {
  ...
  const setRechagurl = () => {
    if (!edit2) {
      ...
      Api(`apiBox/dashboard/updateRechargeCallbackUrl`, 'POST', body, token).then(
        (Response: any) => {
          console.log('======userProfileInfo==response=====>', Response);
          if (Response.status == 200) {
            if (Response.data.code == 200) {
  ...
  const setTransferUrl = () => {
    if (!edit4) {
      ...
      Api(`apiBox/dashboard/updateTransferCallbackUrl `, 'POST', body, token).then(
        (Response: any) => {
          console.log('======userProfileInfo==response=====>', Response);
          if (Response.status == 200) {
            if (Response.data.code == 200) {
  ...
  const setPayoutUrl = () => {
    if (!edit5) {
      ...
      Api(`apiBox/dashboard/updatePayoutPaymentCallbackUrl `, 'POST', body, token).then(
        (Response: any) => {
          console.log('======userProfileInfo==response=====>', Response);
          if (Response.status == 200) {
            if (Response.data.code == 200) {
  ...
  const setPayoutBene = () => {
    if (!edit7) {
      ...
      let token = localStorage.getItem('token');
      let body = {
        beneVerifyCallbackUrl: url7,
      };
      Api(
        `apiBox/dashboard/
updateBeneVerificationPayoutCallbackUrl`,
        'POST',
        body,
        token
      ).then((Response: any) => {
        ...
        if (Response.status == 200) {
          if (Response.data.code == 200) {
            ...
            setEdit7(!edit7);
            getBBPSUurl();
            console.log('======updateBeneVerificationPayoutCallbackUrl==code 200=====>', Response.data.data);
          } else {
            console.log('======updateBeneVerificationPayoutCallbackUrl=======>' + Response);
          }
        }
      });
    }
  ...
  const setTransferBene = () => {
    if (!edit8) {
      setEdit8(!edit8);
    } else {
      let token = localStorage.getItem('token');
      let body = {
        transferCallbackUrl: url8,
      };
      Api(
        `apiBox/dashboard/
updateBeneVerificationTransferCallbackUrl`,
        'POST',
        body,
        token
      ).then((Response: any) => {
    ...
    Api(`apiBox/dashboard/callbackUrls`, 'GET', '', token).then((Response: any) => {
      ...
      if (Response.status == 200) {
        if (Response.data.code == 200) {
          ...
          setUrl7(Response.data.data.partnerCallbackUrls.payoutBeneVerificationCallback);
          setUrl8(Response.data.data.partnerCallbackUrls.upiBeneVerificationCallback);
          setIsLoading(false);
          console.log('======userProfileInfo==code 200=====>', Response.data.data);
        } else {
          console.log('======userProfileInfo_error=======>' + Response);
    ...
    <>
      ...
      <Stack flexDirection={'row'} gap={15}>
        <Stack>
          ...
          <Box sx={{ mt: 2, width: { xs: '100%', md: '100%' } }}>
            <Stack justifyContent={'space-between'}>
              <Typography variant="h4">Upi BeneVerification Callback URL</Typography>
              <Stack flexDirection={'row'} gap={3} marginTop={3}>
                <TextField
                  label="UPI BeneVerification url"
                  placeholder="UPI BeneVerification url"
                  type="string"
                  value={url8}
                  size="small"
                  disabled={!edit8}
                  variant={edit8 ? 'outlined' : 'filled'}
                  onChange={(e) => setUrl8(e.target.value)}
                />
                <LoadingButton variant="contained" onClick={setTransferBene}>
                  {edit8 ? 'Save' : 'Edit'}
                </LoadingButton>
              </Stack>
            </Stack>
        ...
        <Stack>
          ...
          <Box sx={{ mt: 2, width: { xs: '100%', md: '100%' } }}>
            <Stack justifyContent={'space-between'}>
              <Typography variant="h4">Payout BeneVerification Callback URL</Typography>
              <Stack flexDirection={'row'} gap={3} marginTop={3}>
                <TextField
                  label="Payout BeneVerification url"
                  placeholder="Payout BeneVerification url"
                  type="string"
                  value={url7}
                  size="small"
                  disabled={!edit7}
                  variant={edit7 ? 'outlined' : 'filled'}
                  onChange={(e) => setUrl7(e.target.value)}
                />
                <LoadingButton variant="contained" onClick={setPayoutBene}>
                  {edit7 ? 'Save' : 'Edit'}
                </LoadingButton>
              </Stack>
            </Stack>
